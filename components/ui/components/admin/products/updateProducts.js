...
export default function updateProducts({
  ...
  useEffect(() => {
    if (productData && categories.length > 0) {
      ...
      setFormData({
        ...
        parentCategoryId:
          productData.parentCategoryId || categories[0]?.parentCategoryId || "", // Default to the first parent category ID if no match
        // Pricing information
        ...
        productDetails: {
          ...
          additionalProductDetails:
            productData.productDetails?.additionalProductDetails || [],
        },
  ...
  const handleColorChange = (e, index, field) => {
    ...
    const updatedAdditionalDetails = formData.additionalDetails.map(
      (detail) => ({ ...detail })
    );
    if (field === "colorName")
  ...
  const handleImageUpload = (e, colorIndex) => {
    ...
    const updatedAdditionalDetails = formData.additionalDetails.map(
      (detail, index) => ({
        ...
        imagesPreview: Array.isArray(detail.imagesPreview)
          ? [...detail.imagesPreview]
          : [],
      })
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updateResult = await dispatch(
        updateProductData({ id, updatedData })
      ).unwrap();
                      ...
                      {formData.color.map((color, index) => (
                        ...
                        >
                          ...
                          <div className="mt-4">
                            ...
                            {formData.additionalDetails[index]?.images &&
                              formData.additionalDetails[index].images.length >
                                0 && (
                                <aside className="flex flex-row flex-wrap mt-4">
                                  {formData.additionalDetails[index].images.map(
                                    (image, imgIndex) => (
                                      <div
                                        key={imgIndex}
                ...
                <div className="bottom_section absolute z-10 bottom-0 w-full right-0 pt-4 pb-32 lg:pb-4 lg:py-8 px-6 grid gap-4 lg:gap-6 xl:gap-6 md:flex xl:flex bg-gray-50 border-t border-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                  ...
                  <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
                    <button
                      ...
                      type="submit"
                      disabled={updateproductLoading || imageUploading}
                    >
                      <span>
                        {" "}
                        {updateproductLoading || imageUploading
                          ? "Updating..."
                          : "Update Product"}
                      </span>
                    </button>
