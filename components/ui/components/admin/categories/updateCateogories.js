  ...
  useEffect(() => {
    if (categoryData && parentCategories.length > 0) {
      ...
      setFormData({
        ...
        parentCategory:
          matchingParentCategory?.name || parentCategories[0]?.name || "", // Default to the first parent category if no match
        parentCategoryId:
          matchingParentCategory?.id || parentCategories[0]?.id || "", // Default to the first parent category ID if no match
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updateResult = await dispatch(
        updateCategoryData({ id, categoryData })
      ).unwrap();
