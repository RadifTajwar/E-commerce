...
export default function updateMainBanner({ id, toggleVisibility, resetId, doneUpdate }) {
    ...
    const handleRemoveImage = (imgIndex) => {
        setFormData((prevData) => {
            const updatedImages = prevData.images.filter((_, index) => index !== imgIndex);
            return {
                ...prevData,
                                                    ...
                                                    {formData.images.slice(2).map((image, imgIndex) => (
                                                        <div key={imgIndex} className="relative inline-flex items-center">
                                                            <img
                                                                ...
                                                                src={image instanceof File ? URL.createObjectURL(image) : image} // File object or URL
                                                                alt={`Image ${imgIndex}`}
                                                            />
                                                            <button
                                                                type="button"
