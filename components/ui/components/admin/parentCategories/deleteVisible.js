...
export default function deleteVisible({ id, toggleDeleteVisible, resetId,doneUpdate }) {
    const dispatch = useDispatch();
    console.log("id",id);
     const [categoryName ,setCategoryName] = useState("");
        // Access the category data from the store
        const { parentCategoryData, isLoading: categoryLoading, error: categoryError } = useSelector(
            (state) => state.parentCategoryById
        );
        ...
        useEffect(() => {
            if (parentCategoryData) {
                setCategoryName(parentCategoryData.name);
            }
        }, [parentCategoryData]);
        ...
        const handleSubmitDelete = async (e) => {
            ...
            try {
              const updateResult = await  dispatch(deleteParentCategoryById( id )).unwrap();
            ...
            <div className="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg  sm:max-w-xl custom-modal" role="dialog">
                ...
                <div data-focus-guard="true" tabIndex="1" className="w-px h-0 p-0 overflow-hidden fixed top-px left-px">
                    ...
                    <header className="flex justify-end">
                        <button className="inline-flex items-center justify-center w-6 h-6 text-gray-400 transition-colors duration-150 rounded dark:hover:text-gray-200 hover: hover:text-gray-700" aria-label="close" onClick={resetId}>
                    ...
                    <div className="mb-6 text-sm text-gray-700 dark:text-gray-400 text-center custom-modal px-8 pt-6 pb-4">
                        ...
                        <h2 className="text-xl font-medium mb-2">Are You Sure! Want to Delete <span className="text-red-500">{categoryName}</span>?</h2>
                    ...
                    <footer className="flex flex-col items-center justify-end px-6 py-3 -mx-6 -mb-4 space-y-3 sm:space-y-0 sm:space-x-4 sm:flex-row bg-gray-50 dark:bg-gray-800 justify-center">
                        ...
                        <div className="flex justify-end">
                            <button className="align-bottom inline-flex items-center justify-center cursor-pointer leading-5 transition-colors duration-150 font-medium focus:outline-none px-4 py-2 rounded-lg text-sm text-white bg-blue-500 border border-transparent active:bg-blue-600 hover:bg-blue-600 focus:ring focus:ring-purple-300 w-full h-12 sm:w-auto" type="button"
